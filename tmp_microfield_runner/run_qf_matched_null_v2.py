...
OUT=ROOT/'tmp_microfield_results_qf_null_v2'
...
events=np.array([i for i in kept if maxh<=i<len(x)-maxh],dtype=int)
...
eligible=np.arange(maxh,len(x)-maxh); blocked=np.zeros(len(x),dtype=bool)
...
if len(fallback)==0:raise RuntimeError('no matched-control fallback pool')
...
for p in range(N_PERM):
    ...
    for j,i in enumerate(events):
        pool=pool_by_code.get(int(code[i]),fallback); controls[j]=int(RNG.choice(pool))
...
p_ret=(1+(np.abs(null_ret)>=np.abs(obs_ret)).sum(axis=0))/(N_PERM+1); p_disp=(1+(null_disp>=obs_disp).sum(axis=0))/(N_PERM+1)
q_ret=bh(p_ret); q_disp=bh(p_disp); rows=[]
for k,h in enumerate(HORIZONS):
    rci=boot_ci(event_ret[k]); dci=boot_ci(event_disp[k])
    rows.append({'lag_ms':int(h),'n_events':int(len(events)),'signed_mid_response':float(obs_ret[k]),'signed_mid_response_bp':float(obs_ret[k]*1e4),'return_ci95':rci,'matched_null_return_median':float(np.median(null_ret[:,k])),'p_return':float(p_ret[k]),'q_return':float(q_ret[k]),'imbalance_displacement':float(obs_disp[k]),'imbalance_ci95':dci,'matched_null_displacement_median':float(np.median(null_disp[:,k])),'p_displacement':float(p_disp[k]),'q_displacement':float(q_disp[k]),'response_survives_gate':bool(q_ret[k]<.05 and rci[0] is not None and not(rci[0]<=0<=rci[1]))})
pd.DataFrame(rows).to_csv(OUT/'QF_MATCHED_NULL_RESPONSE.csv',index=False)
pd.DataFrame({'idx':events,'t_ms':x.t_ms.iloc[events].to_numpy(np.int64),'kind':labels,'direction':direction,'score':score[events],'spread':x.spread.iloc[events].to_numpy(float),'depth':x.depth.iloc[events].to_numpy(float),'imbalance':x.imbalance.iloc[events].to_numpy(float),'signed_flow':x.signed_flow.iloc[events].to_numpy(float)}).to_csv(OUT/'QF_EVENTS.csv',index=False)
summary={'source':{'quotes':qmeta,'trades':tmeta},'seed':SEED,'null_replicates':N_PERM,'rows':{'quotes':int(len(q)),'trades':int(len(tr)),'milliseconds':int(len(x))},'events':{'raw_after_refractory':int(len(kept)),'boundary_valid':int(len(events)),'by_kind':{k:int(v) for k,v in pd.Series(labels).value_counts().items()}},'matched_state_dimensions':['spread quartile','top depth quintile','absolute BBO imbalance quintile','trailing-100ms trade activity quintile'],'refractory_ms':100,'boundary_exclusion_ms':maxh,'threshold_abs_robust_z':6.0,'response_gate':'BH-FDR q<0.05 plus bootstrap 95% CI excludes zero','significant_return_horizons_ms':[int(r['lag_ms']) for r in rows if r['response_survives_gate']],'significant_displacement_horizons_ms':[int(r['lag_ms']) for r in rows if r['q_displacement']<.05]}
